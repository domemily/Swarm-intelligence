...
%                       valy=(z(ii,2,jj-1)-z(k,2,jj-1))/dij^2;
                      ...
                      valx=k_2*((1/dij)-(d1/(dij^3)))*(z(k,1,jj-1)-z(ii,1,jj-1))/dij; %近距离排斥远距离吸引
                      valy=k_2*((1/dij)-(d1/(dij^3)))*(z(k,2,jj-1)-z(ii,2,jj-1))/dij; %由ii指向k   
                      
                      datax_11(ii,k)=valx;
                      datay_11(ii,k)=valy;
                  else 
                      valx=k_3*((1/dij)-(d0/(dij^3)))*(z(k,1,jj-1)-z(ii,1,jj-1))/dij; %近距离排斥远距离吸引
        ...
        %-----------------------------------------
          ...
          if z(ii,5,1)==0
              uix(ii)=femalex_00(ii)+femalex_0(ii)-gamma*pix(ii)+1;
              uiy(ii)=femaley_00(ii)+femaley_0(ii)-gamma*piy(ii);
          else 
...
%===============绘制初始位置和最终位置图====================================
   ...
   plot(z(3:n,1,1),z(3:n,2,1),'og');
   hold on 
   plot(z(1:2,1,1),z(1:2,2,1),'*r');
   hold on 
 
   plot(z(1:2,1,end),z(1:2,2,end),'*r','markerfacecolor','r');
   hold on

   plot(z(3:n,1,end),z(3:n,2,end),'og','markerfacecolor','g');
 %========================================================================= 
